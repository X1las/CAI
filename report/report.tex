\documentclass[12pt,a4paper]{article}
\usepackage[margin=2.5cm]{geometry}
\usepackage[utf8]{inputenc}
\usepackage[
  backend=biber,
    style=apa,
    citestyle=authoryear,
    sortcites=true
]{biblatex}
\setlength{\bibitemsep}{0.5em}
\addbibresource{references.bib}
\usepackage{parskip}
\usepackage{xcolor}
\usepackage{graphicx} % Add this package for including images
\usepackage{hyperref} % Add this package for hyperlinks
\usepackage{caption} % Add this package for caption customization
\captionsetup{font=small, margin=0.1\textwidth}
\usepackage{float}
\usepackage{longtable}
\usepackage{titling} % For customizing the title page

\title{Counter Artificial Intelligence - A Preliminary Analysis of the Capabilities of AI Models on Local Systems}
\author{J. Wolffrom}
\date{\today}

% Redefine the \maketitle command to include the logo
\renewcommand{\maketitle}{
  \begin{titlepage}
    \centering
    \vspace*{1cm}
    
    \includegraphics[width=0.4\textwidth]{ruc_logo.png}\\[2cm]
    
    {\LARGE\bfseries\thetitle\par}
    
    \vspace{1.5cm}
    
    {\Large\theauthor\par}
    {\large jwsc@ruc.dk\par}
    \vspace{1cm}
    {\large Supervisor: \par}
    {\large\textit{Hemant Ghayvat}\par}   
    {\large\textit{hghayvat@ruc.dk}\par}
    \vspace{1cm}
    
    {\large\thedate\par}
    \vfill
    
    {\large Roskilde University\par}
    {\large Department of Computer Science\par}
    \vspace{0.5cm}
  \end{titlepage}
}

\begin{document}

\hypersetup{pageanchor=false}
\maketitle
\hypersetup{pageanchor=true}

\begin{abstract}

\input{abstract.tex}

\end{abstract}
\newpage

\tableofcontents
\newpage

\section{Glossary}
\label{Glossary}

\input{glossary.tex}

\section{Disclosure}
\label{Disclosure}

\input{disclosure.tex}

\section{Introduction}
\label{Introduction}

\input{introduction.tex}
\newpage

\section{Linux Commandline}
\label{Commandline}

\input{commandline.tex}
\newpage

\section{The Linux Filesystem}
\label{Filesystem}

\input{filesystem.tex}
\newpage

\section{Analysis}
\label{Analysis}

\input{analysis.tex}
\newpage

\section{Discussion}
\label{Discussion}

\input{discussion.tex}
\newpage

\section{Future Work}
\label{FutureWork}

\input{future_work.tex}
\newpage

\section{Conclusion}
\label{Conclusion}

\input{conclusion.tex}
\newpage

\section{References}
\printbibliography

\newpage
\appendix

\input{appendix.tex}

\end{document}
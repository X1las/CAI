...
The model currently does not account for the possibility of agents using temporary memory allocation to store information about the rival agent or the system state. This is a significant limitation, as it means that the model does not capture the full range of strategies that an agent might employ. For example, an agent could use temporary memory to store a snapshot of the system state at a given time, allowing it to detect changes more quickly and respond more effectively to the rival agent's actions.
...
Now that we have reviewed our major concerns for the model, we can move on to the implications and questions raised by it. The model, while simplified, still raises a number of interesting questions about the nature of agent interactions, the limits of surveillance and countermeasures, and the potential for emergent behaviour in a competitive environment.
